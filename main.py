...
import time
import select
REQUEST_CODE = 8    ##code for echo_request in icmp header
pack_sent = 0
pack_rec = 0
round_trip_times = []
target_host = ""

    ##function to calcualte checksum for icmp header with the help of dummy header and the data
def get_checksum(packet):
    cs = 0 #checksum
    packget_len = (len(packet) // 2) * 2    #if length is odd then subtracting one
    count = 0

    while count < packget_len:  ##adding subsequent pair of bytes
        thisVal = int(packet[count+1]) * 256 + int(packet[count])
        cs = cs + thisVal
        cs = cs & 0xffffffff
        count = count + 2

    if packget_len < len(packet):   ##packet length was odd then condition becomes true
...
def receive(sock, ID, timeout,sequence_no):
    ...
    while 1:
        ...
        if(p_id == ID): #confirming about process id
            ...
            src_ip = str(network_header[12])+"."+str(network_header[13])+"."+str(network_header[14])+"."+str(network_header[15])
            return "\t{} bytes from {}: icmp_seq={} ttl={} time={} ms".format(str(data_length),src_ip,str(sequence),str(timetolive),str(rtt))
...
def get_stats():
    ...
    print("\tPackets: Sent = {}, Received = {}, Lost = {} ({}% loss)".format(str(pack_sent),str(pack_rec),str(loss),str(percent_loss)))
    print("Approximate round trip times in milli-seconds:")
    print("\tMinimium = {}ms, Maximum = {}ms, Average = {}ms ".format(str(min_rtt),str(max_rtt),str(avg_rtt)))
    ##-----------------------------END----------------------------------------##

##---------------------Main---------------------------------------
ping("www.google.com")   ##Pinging on the passed url
get_stats()     ##printing stats after pinging
pack_rec = 0
pack_sent = 0
round_trip_times = []
